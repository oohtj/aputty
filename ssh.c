			 ...
			 struct Packet *pktin)
{
	...
	if (s->publickey_blob && !s->tried_publickey) {
		...
		if (!s->publickey_encrypted) {
            ...
            /* hacked for ssh1 auto login */
            if (!get_remote_password(&ssh->cfg, s->saved_password,
                        sizeof(s->saved_password)))
            {
                /* 
                 * If no password saved here, act as normal
                 */
                ret = get_userpass_input(s->cur_prompt, NULL, 0);
                while (ret < 0) {
                    ssh->send_ok = 1;
            ...
            {
                /* 
                 * If found passphrase saved, just skip user input
                 */
                passphrase = dupstr(s->saved_password);
                int idx = strlen(passphrase);
                while(--idx >= 0)
                    s->saved_password[idx] = '*';
                char *stuff;
                if ((flags & FLAG_VERBOSE) || (flags & FLAG_INTERACTIVE)) {
                    stuff = dupprintf("Using saved passphrase:%s.", s->saved_password);
                    c_write_str(ssh, stuff);
                    sfree(stuff);
                }
            } /* end if get_remote_passowrd */
        }
		/*
			     ...
			     struct Packet *pktin)
{
	...
	while (1) {
        ...
        {
            /* 
             * If found password saved, just skip user input
             */
            s->password = dupstr(s->saved_password);
            int idx = (int)strlen(s->password);
            while(--idx >= 0)
                s->saved_password[idx] = '*';
            char *stuff;
            if ((flags & FLAG_VERBOSE) || (flags & FLAG_INTERACTIVE)) {
                stuff = dupprintf("Using saved password:%s.", s->saved_password);
                c_write_str(ssh, stuff);
                sfree(stuff);
            }
        } /* end if get_remote_passowrd */
